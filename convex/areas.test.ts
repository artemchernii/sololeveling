...
describe('order and colour', () => {
  ...
  test('silver is kept, and cleared', async () => {
    const t = as(ME)
    await t.mutation(api.areas.ensure, {})
    const bodyOf = async () =>
      (await t.query(api.areas.list, {})).find((a) => a.slug === 'body')!
    await t.mutation(api.areas.setSilver, { slug: 'body', silver: true })
    expect((await bodyOf()).silver).toBe(true)
    await t.mutation(api.areas.setSilver, { slug: 'body', silver: false })
    expect((await bodyOf()).silver).toBeUndefined()
  })

  test("another owner's area cannot be made silver", async () => {
    const { mine: me, theirs } = twoOwners()
    await me.mutation(api.areas.ensure, {})
    await me.mutation(api.areas.create, { label: 'Chess' })
    await expect(
      theirs.mutation(api.areas.setSilver, { slug: 'chess', silver: true }),
    ).rejects.toThrow('NO_SUCH_AREA')
    const chess = (await me.query(api.areas.list, {})).find(
      (a) => a.slug === 'chess',
    )!
    expect(chess.silver).toBeUndefined()
  })
})
