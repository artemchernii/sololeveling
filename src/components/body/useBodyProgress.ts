...
import { lookupRef } from '@/lib/body/library'
...
export function useBodyProgress(): BodyProgress {
  /* Fixed at mount so the query's arguments do not change every render. */
  ...
  return {
    ...
    pickedPrograms,
    pickedDays,
    pick: nextRoutine(pickedDays, (id) => lastByDay.get(id) ?? null, today),
